...
class RenderLatexFenceActionEndToEndLightTest : LightPlatformTestCase() {
  fun testActionPerformsCompileWithoutShowingUi() {
    org.junit.Assume.assumeTrue(System.getProperty("enableLightTests", "false").toBoolean())
    val md = """
      # T
    ...
    try {
      // Move caret inside LaTeX fence
      editor.caretModel.moveToOffset(md.indexOf("E = mc"))

      // Mock LocalFileSystem lookup to return a non-null vfile for the temp .tex path
      mockkStatic(LocalFileSystem::class)
      val lfs = mockk<LocalFileSystem>(relaxed = true)
      every { LocalFileSystem.getInstance() } returns lfs
      every { lfs.refreshAndFindFileByIoFile(any()) } answers {
        mockk<VirtualFile>(relaxed = true)
      }

      // Replace LatexCompilerService with a mock that returns a fake PDF path
      ...
      // Prevent UI from showing by intercepting invokeLater
